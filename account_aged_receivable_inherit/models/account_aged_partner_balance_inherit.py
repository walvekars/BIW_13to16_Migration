...
class report_account_aged_receivable(models.AbstractModel):
    ...
    @api.model
    def _init_filter_all(self, options, previous_options=None):
        if not self.filter_all:
            return
        options['filter_all'] = True
...
class ReportAgedPartnerBalance(models.AbstractModel):
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move, period_length, analytic_ids=None):
        ...
        for i in range(6):
            ...
            if analytic_ids:
                args_list = (tuple(move_state), tuple(account_type), tuple(partner_ids),tuple(analytic_ids))
        ...
        if not analytic_ids:
            ...
            query = '''SELECT l.id
                        ...
                        AND l.company_id IN %s
                        ORDER BY COALESCE(l.date_maturity, l.date)'''
            cr.execute(query, (tuple(move_state), tuple(account_type), date_from, tuple(partner_ids),
                               tuple(analytic_ids), date_from, tuple(company_ids)))
        aml_ids = cr.fetchall()

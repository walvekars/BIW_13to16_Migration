...
class report_account_general_ledger_cost(models.AbstractModel):
    _inherit = "account.general.ledger.cost"
            ...
            sql = """WITH account_move_line AS (
              ...
              (
               WITH payment_table AS (
                 SELECT aml.move_id, \"account_move_line\".date,
                   ...
                   WHERE part.credit_move_id = "account_move_line".id
                    AND "account_move_line".account_type IN %s
                    AND """ + where_clause + """
                 UNION ALL
                 SELECT aml.move_id, \"account_move_line\".date,
                        CASE WHEN (aml.balance = 0 OR sub_aml.total_per_account = 0)
               ...
               SELECT aml.id, ref.date, aml.name,
                 ...
                 aml.amount_residual,aml.cost_center_id, aml.partner_id, aml.reconciled, aml.company_id, aml.company_currency_id, aml.amount_currency, aml.balance, aml.account_type, aml.analytic_account_id
                ...
                WHERE journal_id NOT IN (SELECT id FROM account_journal WHERE type in ('cash', 'bank'))
                  AND aml.move_id IN (SELECT DISTINCT move_id FROM account_move_line WHERE account_type IN %s)
              )
            ) """
    ...
    def _do_query_unaffected_earnings(self, options, line_id, company=None):
        ...
        user_types = self.env['account.account'].search([('type', 'in', ('receivable', 'payable'))])
        with_sql, with_params = self._get_with_statement(user_types, domain=[('account_id.account_type.include_initial_balance', '=', False)])
        aml_domain = [('account_id.account_type.include_initial_balance', '=', False)]
        if company:
    ...
    def _do_query(self, options, line_id, group_by_account=True, limit=False):
        ...
        if group_by_account:
            ...
            if limit and isinstance(limit, int):
                sql += " LIMIT " + str(limit)
        user_types = self.env['account.account'].search([('account_type', 'in', ('asset_receivable', 'liability_payable'))])
    ...
    def _group_by_account_id(self, options, line_id):
        ...
        for account_id, result in results.items():
            ...
            if account.account_type == unaffected_earnings_type and account.company_id not in unaff_earnings_treated_companies:
        ...
        for cid in context.get('company_ids', []):
            ...
            if company not in unaff_earnings_treated_companies and not float_is_zero(unaffected_earnings_per_company[company]['balance'], precision_digits=user_currency.decimal_places):
                unaffected_earnings_account = self.env['account.account'].search([
                    ('account_type', '=', unaffected_earnings_type.id), ('company_id', '=', company.id)
                ], limit=1)
    ...
    #
    #     var_dict = list(line_id)
    #     # line_id = line_id and int(line_id.split(',')[1]) or None

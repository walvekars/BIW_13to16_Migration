...
class AccountStandardLedgerLines(models.TransientModel):
    ...
    #     result = super(AccountStandardLedgerLines, self).default_get(fields)
    #     model = self.env.context.get('active_model')
    #     active_id = self.env.context.get('active_id')

    #     print(model, active_id,'**************************************')
    ...
    category_id = fields.Many2many(related='partner_id.category_id', string="Tags")
    # cat_name = fields.Char(related='partner_id.category_id.name', string="cat ")
    tags_name = fields.Char(string="Partner Tags",store=True)
    group_by_key = fields.Char()
    move_id = fields.Many2one('account.move', 'Entry')
    reference = fields.Char(related='move_id.ref', string="Reference")
...
class AccountStandardLedger(models.TransientModel):
    ...
    def action_view_lines(self):
        self.ensure_one()
        self._compute_data()
        report_rec = self.env['account.report.standard.ledger.line'].sudo().search([('report_id','=',self.report_id.id)])
        if report_rec:
            for lines in report_rec:
                lines.write({'tags_name':str(lines.partner_id.category_id.mapped('name')).strip("[']")})
        tree_view_name =  self.report_id.name
        if self.ledger_type == 'aged':
            if self.result_selection == 'supplier':
                tree_view_name = 'Supplier Aging Report'
            elif self.result_selection == 'customer':
                tree_view_name = 'Customer Aging Report'
    ...
    def _sql_unaffected_earnings(self):
        ...
        unaffected_earnings_account = self.env['account.account'].search(
            [('company_id', '=', company.id), ('account_type', '=', self.env.ref('account.data_unaffected_earnings').id)], limit=1)
        ...
        FROM
            ...
            LEFT JOIN account_account_type acc_type ON (acc.account_type = acc_type.id)
    ...
    def _sql_lines(self):
        ...
        params = [
            ...
            self.report_type,
            tuple(self.partner_ids.ids) if self.partner_ids else (None,),
            # tuple(str(self.partner_ids.category_id.mapped('name')).strip("[']")) if self.partner_ids.category_id else (None,),
            self.report_type,
            tuple(self.analytic_account_ids.ids) if self.analytic_account_ids else (None,),
            self.compact_account,
            self.reconciled,
